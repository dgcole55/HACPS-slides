...
## Types of Objects in seL4 (Managed by Capabilities)
- **Endpoints** For function calls and communication.
- **Address Spaces** To ensure isolation.
- **Scheduling Contexts** For CPU time allocation.
- Other objects support memory, threads, and device management.
...
## Example Confinement of Untrusted Programs
...
- **Scenario**
  - A compiler collects usage statistics, storing them in a protected file `(SYSX)STAT`.
  - As such, the compiler is given write privileges to `(SYSX)`.
  - Users can specify an output file for compilation results.

- **Exploit**
  - A user specifies `(SYSX)BILL` (a sensitive billing file) as the output.
  - The compiler, having write access to `SYSX`, overwrites `(SYSX)BILL`, corrupting billing data.
...
- **Authority Misuse**
  - The compiler uses its own permissions rather than the user's, leading to unintended access.
...
## How Capabilities Solve This Problem

- **Coupling Denomination and Authority**
  - A capability references an object and encapsulates its **access rights**.
  - Operations require the user to provide explicit capabilities.
  
- **Example**
  - A compiler in a capability-based system
    - Operates only within the authority granted explicitly by the user.
    - Cannot access or modify resources beyond its designated scope.
...
- **RISC-V Opportunity**
  - Open-source RISC-V processors enable reapplying WCET analysis.
  - Reinforces seL4's strength in **real-time** and **safety-critical systems**.
...
Systems with components of varying **criticality levels**

- **Criticality** Severity of consequences in case of a failure.
- Example Avionics classify failures from “no effect” to “catastrophic.”
...
- **Strict Isolation**
  - Fixed memory areas and dedicated time slices for each component.
  - Guarantees **spatial and temporal isolation**.
...
- **Configuration**
  ...
  - High-Priority Driver
    - Smaller budget and shorter period.
